// Ручной резервный способ обновить остатки — основной путь теперь веб-форма
// на /admin/stock (менеджер/админ), которая использует ту же логику разбора
// файла, продублированную в lib/stockImport.ts (TS/алиасы там не дают
// напрямую переиспользовать код из этого CLI-скрипта). Меняя правила
// разбора (NAME_FIXES, транслит, порядок колонок), правь оба места.
